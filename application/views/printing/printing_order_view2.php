...
<div class="row profile-page">
    ...
    <?php if($value1['item_unit_qty_input']!=0){ $i++?>
		...
		$st='<span class="badge badge-outline-danger" >'.$any_rejection['rejected_department'].' Rejected :<br/>'.$any_rejection['verify_remark'].'<br>'.$any_rejection['verify_datetime'];
		if($any_rejection['rejected_count']!=0){
		$st.="<br>Rejected Item Count: ".$any_rejection['rejected_count'];
		}
		$st.='</span>';
			...
			if(isset($lastUpdateRow)){
				if($lastUpdateRow['verify_status']==1){
					...
					if($myUpdation){
						...
						$st='<br/><span class="badge badge-outline-danger mt-1 w-100" >'.ucwords($myUpdation['row_status']).'</span>';
					}else{
						$up='&nbsp;<a href="#" title="Update Status" style="cursor: pointer;"  data-toggle="modal" data-target="#orderItemUpdate"  data-sid="'.$schedule_data['schedule_id'].'"  data-sdid="'.$schedule_data['schedule_department_id'].'" data-smid="'.$value1['summary_id'].'" data-did="'.$staffRow['department_id'].'" data-act="up" ><label class="badge badge-primary w-100" style="cursor: pointer;">Update Status</a>';
					}
				}
				if($lastUpdateRow['verify_status']==-1){
