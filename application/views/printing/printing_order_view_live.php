...
<div class="row profile-page">
    ...
    <?php foreach($array1 as $key1 => $value1){?>
		...
		$lastUpdateRow=$this->myaccount_model->get_last_updated_row_by_schedule_id($schedule_data['schedule_id'],$value1['summary_id'],'design','design_qc');
		...
		if(isset($any_rejection)){
			...
			if(isset($lastUpdateRow)){
				if($lastUpdateRow['verify_status']==1){
					...
					$myUpdation=$this->myaccount_model->get_last_updated_row_by_schedule_id($schedule_data['schedule_id'],$value1['summary_id'],'printing','fusing');
					if($myUpdation){
						if($actionFrom=="completed"){$rowNow="completed";}
						$approvedQty=$value1['item_unit_qty_input']-$Rqty;
						$st='<br/><span class="badge badge-outline-success mt-1 w-100 text-info" >'.ucwords($myUpdation['row_status']).'<br/> To Fusing</span>';
					}else{
    	...
    	<td style="font-size:10px;">
            ...
            $sql="Select * from rs_design_departments where summary_item_id='".$value1['summary_id']."' and from_department='design' and schedule_id='".$schedule_data['schedule_id']."' ORDER BY rs_design_id DESC LIMIT 1";  
            //echo $sql;
            $query = $this->db->query($sql);					 
