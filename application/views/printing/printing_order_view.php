...
$rejection_is_set=0;
		...
		$lastUpdateRow=$this->myaccount_model->get_last_updated_row_by_schedule_id_by_field($schedule_data['schedule_id'],$value1['summary_id'],'design','design_qc');
		//$any_rejection=$this->myaccount_model->check_any_rejection($schedule_data['schedule_department_id'],$value1['summary_id']);
		$any_rejection=$this->myaccount_model->check_any_rejection_by_qty_field($schedule_data['batch_number'],$schedule_data['schedule_id'],$value1['summary_id'],5);
		if(isset($any_rejection)){
			...
			if(isset($lastUpdateRow)){
				if($lastUpdateRow['verify_status']==1){
					$st='<span class="badge badge-outline-success w-100 text-info" > QC Approved</span>';
					$myUpdation=$this->myaccount_model->get_last_updated_row_by_schedule_id_by_field($schedule_data['schedule_id'],$value1['summary_id'],'printing','fusing');
					if($myUpdation){
						if($actionFrom=="completed"){$rowNow="completed";}
						$approvedQty=$value1['item_unit_qty_input']-$Rqty;
						$st='<br/><span class="badge badge-success mt-1 w-100" >In Fusing</span>';
					}else{
						$st='<br/><span class="badge badge-success mt-1 w-100" >In Printing</span>';
						$up='<a href="#" title="Update Status" style="cursor: pointer;"  data-toggle="modal" data-target="#orderItemUpdate"  data-sid="'.$schedule_data['schedule_id'].'"  data-sdid="'.$schedule_data['schedule_department_id'].'" data-smid="'.$value1['summary_id'].'" data-did="'.$staffRow['department_id'].'" data-act="up" ><label class="badge badge-primary w-100" style="cursor: pointer;">Update Status</a><br />';
					}
				}
				if($lastUpdateRow['verify_status']==-1){
					$st='<span class="badge badge-danger w-100" >Rejectd</span>';
				}
				if($lastUpdateRow['verify_status']==0){
					if($actionFrom=="pending"){$rowNow="pending";}
					$st='<span class="badge badge-danger w-100" >In Design QC</span>';
				}
			}else{
				if($actionFrom=="pending"){$rowNow="pending";}
				$st='<span class="badge badge-warning w-100" >In Design QC</span>';
			}
			}
		//}
