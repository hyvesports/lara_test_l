...
<?php
$sql="Select * from wo_work_orders where order_id='".$schedule_data['order_id']."' ";  
$query = $this->db->query($sql);					 
$workOrderRow= $query->row_array();
?>
<?php if($workOrderRow['wo_product_info']!="") {?>
<div class="card card-inverse-secondary mb-1">
<div class="card-body">
<p class="mb-1"><?php echo $workOrderRow['wo_product_info'];?></p>
</div>
</div>
<?php } ?>
...
<?php //print_r($schedule_data);?> 
    ...
    <div class="col-md-12 grid-margin stretch-card mt-1">
...
<?php if($value1['img_front']!=""){?>
   ...
   <?php $options=$this->myaccount_model->get_order_options($schedule_data['order_id'],$value1['summary_id']); ?>
	...
	<th class="text-right">Fit Type</th>
	...
	<td><?php echo $OP['fit_type_name'];?></td>
	<td><?php echo $OP['option_qty'];?></td>
    </tr>
