...
if($pORc!=0) {
	//echo $pORc;
	$refData = $this->workorder_model->get_summary_item_data($pORc);
	$wo_ref_no=$refData['wo_ref_no'];
	$readonly='readonly="readonly"';
} 
 $product_fit = $this->workorder_model->get_product_fit();
?>
<form action="post" id="mformNew">
<input type="hidden" id="customer_id" name="customer_id" />
<input type="hidden" id="summary_parent" name="summary_parent" value="<?php echo $pORc;?>" />
<input type="hidden" id="wo_qty" name="wo_qty" value="1" />
<div class="modal-body">
<span id="responseNew"></span>
<?php if($pORc==0) {?>
...
<div class="row">
    ...
    <div class="form-group">
    <input type="text" class="form-control" id="customer_mobile_no" name="customer_mobile_no" placeholder="Mobile Number" maxlength="10" minlength="10" onkeyup="chkphonenumber_cust(this.value);">
    </div>
    </div>
...
<div class="row">
    ...
    <div class="form-group">
    <input type="text" class="form-control" id="wo_ref_no" name="wo_ref_no" placeholder="Reference Number" maxlength="50" minlength="1" value="<?php echo $wo_ref_no;?>"  <?php echo $readonly;?>>
    </div>
    </div>
...
<input type="hidden" name="wo_option_session_id" id="wo_option_session_id" value="<?php echo $_POST['option_session_id'];?>" />
...
$.post('<?= base_url("workorder/save_new_summary/");?>', $("#mformNew").serialize(), function(data) {
	...
	if(dataRs.responseCode=="success"){ 
		...
		var post_data = {
		'<?php echo $this->security->get_csrf_token_name(); ?>' : '<?php echo $this->security->get_csrf_hash(); ?>',
		'<?php echo 'wo_order_id'; ?>' : '<?php echo $order_id;?>'
		};
		$.ajax({
		type: "POST",
		...
		$("#responseNew").html(dataRs.responseMsg);
	}
});	
}
});	
