...
if( $_POST['did']==4){ // design
	...
	//4
	//echo $sdepartment['department_schedule_date'];
	...
	//echo 'stitching';
	$pRowArray=$this->schedule_model->get_previous_department_and_date(8,$sdepartment['order_id'],$sdepartment['unit_id'],$sdepartment['department_schedule_date']);
	$minDate=date('d-m-Y', strtotime($pRowArray['department_schedule_date']));
	$nextDepatartment=10;
	//$maxDate=
	$is_dispatch=1;
...
<h4 class="card-title">Order Items</h4>
    ...
    <?php foreach($orderArray as $ITEM){?>
		...
		//print_r($ITEM['scheduled_order_info']);
		...
		foreach($arrayJson as $jKey => $jValue){
			...
			if($jValue['item_unit_qty_input']!=0){

			//$rejectedArray=$this->common_model->check_is_rejected_new($sdepartment['order_id'],$jValue['summary_id']);

			if(!in_array($jValue['summary_id'], $rej_items_array)){
...
<h4 class="card-title">Order Dates</h4>
    ...
    <input type="text" class="form-control required" id="selected_date" name="selected_date" readonly="readonly"  value="<?php echo $minDate;?>">
