...
<?php 
$orderArray=$this->schedule_model->get_orders_under_date_and_depmt($_POST['CD'],$_POST['did'],$_POST['UID']); ?>
...
<input type="hidden" name="changeDate[<?php echo $K;?>][item_order_capacity]" value='<?php echo ucwords($jValue['item_order_capacity']);?>' /> 
<input type="hidden" name="changeDate[<?php echo $K;?>][item_order_qty]" value='<?php echo ucwords($jValue['item_order_qty']);?>' /> 
<input type="hidden" name="changeDate[<?php echo $K;?>][online_ref_number]" value='<?php echo ucwords($jValue['online_ref_number']);?>' />
...
<input type="hidden" name="orginalData[<?php echo $K;?>][item_order_total_sec]" value='<?php echo ucwords($jValue['item_order_total_sec']);?>' /> <input type="hidden" name="orginalData[<?php echo $K;?>][item_order_capacity]" value='<?php echo ucwords($jValue['item_order_capacity']);?>' /> 
<input type="hidden" name="orginalData[<?php echo $K;?>][item_order_qty]" value='<?php echo ucwords($jValue['item_order_qty']);?>' /> 
<input type="hidden" name="orginalData[<?php echo $K;?>][online_ref_number]" value='<?php echo ucwords($jValue['online_ref_number']);?>' /> 
        ...
        <p class="text-muted mb-0">Ref No: <?php echo ucwords($jValue['online_ref_number']);?></p>
        </td>
        <?php $order_total_qty+=$jValue['item_unit_qty_input'];?>
        <td class="text-center"><?php echo ucwords($jValue['item_unit_qty_input']);?></td>
        ...
        <input type="text" class="form-control text-center required digits" name="orginalData[<?php echo $K;?>][item_unit_qty_input]" id="item_unit_qty_input1<?php echo $K;?>" readonly min='0' max='<?php echo ucwords($jValue['item_unit_qty_input']);?>' />
...
function get_balance_qty(current_row,row_value,old_value){
   ...
   $(function() {
      $('#selected_date').daterangepicker({
		...
		showDropdowns: false,
		minDate: "<?php echo date('d-m-Y', strtotime($sdepartment['department_schedule_date']));?>"
      }, 
     function() {
