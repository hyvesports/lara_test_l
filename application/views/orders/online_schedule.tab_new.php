...
$proDate=$this->schedule_model->get_production_start_date_from_calendar_for_online(date('Y-m-d'));
?>
<form action="post" id="scheduleInputForm" name="scheduleInputForm">
    ...
    <?php $proDate['START_DATE'];
	$production_start_date=date("d-m-Y", strtotime($proDate['START_DATE']));
	?>
    
    <div class="col-md-2">
    <label>Production Start Date <span class="text-danger">*</span></label>
     <input type="text" class="form-control required datepicker" name="production_start_date" id="production_start_date" value="<?php echo $production_start_date;?>" readonly="readonly" >
    </div>
...
<?php $disDates=$this->schedule_model->get_all_disabiled_days_d_m_y(date('Y-m-d')); ?><?php echo $disDates['CDATE'];?>
<script>
$('.datepicker').datepicker({
autoclose: true,
format: 'dd-mm-yyyy',
startDate: "<?php echo $production_start_date;?>",
<?php if($disDates!=""){ ?>
