...
if($datesRowsCount==$dayLimit) {
	...
	//print_r($datesRows[$dateLastRowStitching]);
	...
	if($AvailableCapacity==0){
		...
		$order_percent=number_format( $percent * 100, 2 ) . '%'; 
		$order_percent_without_round= number_format( $percent * 100, 2 ); 
	...
	if($order_percent_without_round==0){
...
<div class="col-12 mt-4">
    ...
    <div class="row">
        ...
        <a href="javascript:void(0);"  data-toggle="modal" data-target="#summaryModel"  data-ps="<?php echo strtotime($production_start_date);?>" data-pe="<?php echo strtotime($nextdate);?>" style="display:none;">
        ...
        <a href="javascript:void(0);"  data-toggle="modal" data-target="#summaryModel"  data-ps="<?php echo strtotime($production_start_date);?>" data-pe="<?php echo strtotime($futuredate);?>" style="display:none;">
...
<div class="col-md-6">
    ...
    <th>Qty <br/> <label class="badge badge-warning mt-1">Total: <?php echo $line_qty;?></label></th>
    <!--<th>Making Time</th>
    <th>Total Time</th>-->
	...
	//print_r($cuRow);
	?>
    <th width="25%"  class="text-left"><?php echo $PU['production_unit_name'];?>
    <br /><label class="badge badge-warning mt-1">Capacity: <?php echo $AvailableCapacity;?>%</label>
   ...
   <input type="hidden" name="schedule[<?php echo $j;?>][online_ref_number]" id="online_ref_number<?php echo $j;?>" 
   value="<?php echo $item['wo_ref_no'];?>" />
   <input type="hidden" name="schedule[<?php echo $j;?>][item_order_qty]" id="item_order_qty<?php echo $j;?>" value="<?php echo $my_line_qty;?>" />
...
<input type="hidden" name="schedule[<?php echo $j;?>][item_rejected_qty]" id="item_rejected_qty<?php echo $j;?>" value="0" />
    ...
    <?php if($my_line_qty<=0){ ?> readonly="readonly" value="<?php echo $my_line_qty;?>"  <?php }else{?> value="<?php echo $my_line_qty;?>" <?php } ?> >
