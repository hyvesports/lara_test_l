...
<?php endif; ?>
<h4 class="card-title"><?php echo $title;?> | Pending 
...
<a class="btn btn-danger" href="<?=base_url('myaccount/myorders/pending/')?>">Pending</a>
...
$records = $this->design_model->get_design_works_pending_new($staffRow['department_id'],$staffRow['unit_managed']);
//print_r($records);
?>
...
$upSatatus=0;
					...
					$option='&nbsp;<a href="'.base_url('myaccount/order_view/'.$row['schedule_uuid']).'/'.$row['schedule_department_id'].'" title="View" style="cursor: pointer;"><label class="badge badge-info" style="cursor: pointer;"><i class="fa fa-search" ></i></label></a>';
					 ...
					 if(!isset($rsRow2)){
						 ...
						 ?>
					<tr>
                    <td><span class="badge" style="background-color:<?php echo $row['priority_color_code'];?>;" >
					<?php echo date("d-m-Y", strtotime($row['department_schedule_date']));?></span></td>
                    <?php echo '<td><span class="badge badge-success" >'.$row['orderform_number'].' ('.$i.'/'.$total_items.')</span>'.$re.'</td>';?>
                    <?php echo '<td><span class="badge" >'.$row['wo_date_time'].'</span></td>';?>
                    <td><?php echo date("d-m-Y", strtotime($dispatchRow['department_schedule_date']));?></td>
                    <td><?php echo $sales_handler;?></td>
                    <td><?php echo  '<label class="badge badge-outline-success">'.$wo_product_info.$ref.$value1['product_type'].'-'.$value1['item_unit_qty_input'].'</label>';?></td>
                   <td><?php echo $st;?></td>
                    <td><?php echo $info.$us.$option;?></td>
