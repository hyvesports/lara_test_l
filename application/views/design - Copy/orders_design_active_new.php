...
<?php endif; ?>
<h4 class="card-title"><?php echo $title;?> | Active
...
<a class="btn btn-warning" href="<?=base_url('myaccount/myorders')?>">Active</a>
...
$accessArray=$this->rbac->check_operation_access(); 
		...
		$records = $this->design_model->get_design_works_active_new($staffRow['department_id'],$staffRow['unit_managed']);
		//print_r($records);
		$data = array();
...
$upSatatus=0;
					...
					$option='&nbsp;<a href="'.base_url('myaccount/order_view/'.$row['schedule_uuid']).'/'.$row['schedule_department_id'].'" title="View" style="cursor: pointer;"><label class="badge badge-info" style="cursor: pointer;"><i class="fa fa-search" ></i></label></a>';
						...
						echo '<tr><td><span class="badge" style="background-color:'.$row['priority_color_code'].';" >'.date("d-m-Y", strtotime($row['department_schedule_date'])).'</span></td>';
