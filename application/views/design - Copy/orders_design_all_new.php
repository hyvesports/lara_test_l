...
$accessArray=$this->rbac->check_operation_access(); 
		...
		$records = $this->design_model->get_design_works_all_new($staffRow['department_id'],$staffRow['unit_managed']);
		//print_r($records);
		$data = array();
		...
		foreach ($records as $row) 
		{
			...
			$array1 = json_decode($row['scheduled_order_info'],true);
			 ...
			 $i=0;if($array1) {
				...
				//echo ucwords($value1['item_position']. $value1['item_sub'])."<br/>";
...
$upSatatus=0;
					...
					$option='&nbsp;<a href="'.base_url('myaccount/order_view/'.$row['schedule_uuid']).'/'.$row['schedule_department_id'].'" title="View" style="cursor: pointer;"><label class="badge badge-info" style="cursor: pointer;"><i class="fa fa-search" ></i></label></a>';
						...
						echo '<tr><td id='.$i.'><span class="badge" style="background-color:'.$row['priority_color_code'].';" >'.date("d-m-Y", strtotime($row['department_schedule_date'])).'</span></td>';
