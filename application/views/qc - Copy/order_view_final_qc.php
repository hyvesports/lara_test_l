...
<div class="row profile-page">
    ...
    <div id="lightgallery2" class="row lightGallery"><a href="<?php echo $value1['img_front'];?>" target="_blank"><img src="<?php echo $value1['img_front'];?>" title="Image Front" width="50" height="50" ><br />Front</a></div>
    </span>
    <?php } ?>
    <?php if($value1['img_back']!=""){?>
    <span class="btn btn-xs btn-outline-primary mr-2 mt-1">
    <div id="lightgallery2" class="row lightGallery"><a href="<?php echo $value1['img_back'];?>" target="_blank"><img src="<?php echo $value1['img_back'];?>" title="Image Back" width="50" height="50"  ><br/>Back</a></div>
    </span>
    <?php } ?>
   ...
   <?php $options=$this->myaccount_model->get_order_options($row['order_id'],$value1['summary_id']); ?>
	...
	$qc_result=$this->qc_model->check_stitching_completed($schedule_data['schedule_department_id'],$value1['summary_id'],'stitching');

	if($qc_result['rs_design_id']){ ?>

    	

		<?php if($qc_result['approved_dep_name']!=""){ ?>

        <span class="badge badge-outline-success w-100" >

        <?php echo $qc_result['approved_dep_name'];?> submitted to Dispatch department <br />

        Remark : <?php echo $qc_result['verify_remark'];?>
    ...
    <?php $design_result=$this->bundling_model->check_bundling_completed_new($value1['summary_id'],'design');?>
