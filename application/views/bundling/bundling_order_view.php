	...
	<tbody>
		...
		<?php foreach($array1 as $key1 => $value1){
			...
			if(isset($any_rejection)){
				...
				$any_rejection['rejected_from_departments'].' Rejected ';
				...
				if(isset($lastUpdateRow)){
					if($lastUpdateRow['verify_status']==1){
						...
						$st='<span class="badge badge-outline-success text-info w-100" >In Stitching</span>';
					}
					if($lastUpdateRow['verify_status']==-1){
						$rejection_count++;
						$approvedQty=$value1['item_unit_qty_input']-$Rqty;
					...
					$approvedQty=0;
					$not_submitted_count++;
					$st='<span class="badge badge-outline-warning w-100" >In Fusing</span>';
					if($actionFrom=="pending"){$rowNow="pending";}
				}
				}
			//}
