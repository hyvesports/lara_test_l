...
<?php endif; ?>
<h4 class="card-title">Update Task <span style="text-align:right; float:right;"><a class="btn btn-primary " href="<?php echo base_url('leads/index');?>">All Leads</a></span> </h4>

<?php echo form_open(base_url('leads/task_edit/'.$task_info['task_uuid']), 'class="cmxform"  id="taskForm" method="post"'); ?>
    <input type="hidden" name="<?php echo $this->security->get_csrf_token_name(); ?>" value="<?php echo $this->security->get_csrf_hash(); ?>">
    <input type="hidden" name="lead_id"  id="lead_id" value="<?php echo $lead_info['lead_id'];?>" />
     <input type="hidden" name="task_id"  id="task_id" value="<?php echo $task_info['task_id'];?>" />
    <div class="row">
    
    <div class="col-md-12">
    <label>Task <span class="text-danger">*</span></label>
     <input type="text" class="form-control required" name="task_desc" id="task_desc" value="<?php echo $task_info['task_desc'];?>"  >
    </div>
    <div class="col-md-3">
    <label>Reminder Date <span class="text-danger">*</span></label>
    <div id="datepicker-popup" class="input-group date datepicker">
	<input type="text" class="form-control required"  name="reminder_date"  id="reminder_date"  value="<?php echo date("d-m-Y",strtotime($task_info['reminder_date']));?>">
    ...
    <div class="col-md-6"> <button type="submit" style="margin-top:30px; width:100%;"  class="btn btn-info" id="submitData" name="submitData" value="submitData">Update Task</button>
    </div>
    </div>
    </form>
</div>
