...
<?php
//echo $offline_draft_id;

$refData= $this->formdata_model->get_summary_of_online_data($order_summary_id);
//print_r($refData);
$wo_order_id=$refData['wo_order_id'];
$wo_option_session_id=$refData['wo_option_session_id'];
$order_summary_id=$refData['order_summary_id'];
?>
<?php $options = $this->formdata_model->get_all_summary_options_latest($order_summary_id,$wo_order_id); ?>
<form action="post" id="mform">
<input type="hidden" name="order_summary_id" id="order_summary_id" value="<?php echo $order_summary_id;?>" />
<div class="modal-body">
<span id="response"></span>

<h5 class="card-title">Order Details edit </h5>  
...
<div class="row">
    ...
    <option value="<?php echo $PT['product_type_id'];?>" ovalue='<?php echo $PT['product_type_amount'];?>' <?php if($PT['product_type_id']==$refData['wo_product_type_id']){ echo 'selected="selected"';}?> ><?php echo $PT['product_type_name'];?> (<?php echo $PT['product_type_amount'];?>)</option>
    <?php } ?>
    <?php } ?>
    </select>
    </div>
    ...
    <option value="<?php echo $CT['collar_type_id'];?>"  ovalue='<?php echo $CT['collar_amount'];?>' <?php if($CT['collar_type_id']==$refData['wo_collar_type_id']){ echo 'selected="selected"';}?>><?php echo $CT['collar_type_name'];?> (<?php echo $CT['collar_amount'];?>)</option>
    <?php } ?>
    <?php } ?>
    </select>
    </div>
    ...
    <option value="<?php echo $ST['sleeve_type_id'];?>"  ovalue='<?php echo $ST['sleeve_type_amount'];?>' <?php if($ST['sleeve_type_id']==$refData['wo_sleeve_type_id']){ echo 'selected="selected"';}?> ><?php echo $ST['sleeve_type_name'];?> (<?php echo $ST['sleeve_type_amount'];?>)</option>
    <?php } ?>
    <?php } ?>
    </select>
    </div>
    ...
    <option value="<?php echo $FT['fabric_type_id'];?>" <?php if($FT['fabric_type_id']==$refData['wo_fabric_type_id']){ echo 'selected="selected"';}?> ovalue='<?php echo $FT['fabric_amount'];?>'><?php echo $FT['fabric_type_name'];?> (<?php echo $FT['fabric_amount'];?>)</option>
    <?php } ?>
    <?php } ?>
    </select>
    </div>
...
<input type="hidden" name="wo_addon_id" id="wo_addon_id" value="0" />
    ...
    <?php if($addons){ ?>
	<?php foreach($addons as $AD){ ?>
    <option value="<?php echo $AD['addon_id'];?>" advalue='<?php echo $AD['addon_amount'];?>' <?php if(in_array($AD['addon_id'],$wo_item_addons)){ echo 'selected="selected"';}?>><?php echo $AD['addon_name'];?> (<?php echo $AD['addon_amount'];?>)</option>
    <?php //} ?></option>
 	<?php }} ?>
    </select>
    </div>
...
<div class="row">
    ...
    <label>Rate</label>
    <div class="form-group">
    <input type="text" class="form-control" id="wo_rate" name="wo_rate" readonly="readonly" value="<?php echo $refData['wo_rate'];?>"  >
    </div>
    </div>
  ...
  <div class="col-md-3">
    ...
    <div class="form-group">
    <input type="text" class="form-control number" id="wo_discount" name="wo_discount"  maxlength="5"  value="<?php echo $refData['wo_discount'];?>" >
    </div>
    </div>
...
<input type="hidden" name="wo_qty" id="wo_qty_total" value="<?php echo $refData['wo_qty'];?>" />
<input type="hidden" name="order_id" id="order_id" value="<?php echo $_POST['order_id'];?>" />
<input type="hidden" name="current_s_id" id="current_s_id" value="<?php echo $refData['wo_option_session_id'];?>" />
...
$('#editRefNo').on('hidden.bs.modal', function () { 
	...
	'<?php echo 'order_id'; ?>' : '<?php echo $_POST['order_id']; ?>'
	};
	//$('#pnoresponse').html("<p style='text-align:center; color:#030;'>Checking... </p>");
	$.ajax({
	type: "POST",
	url: "<?= base_url("formdata/summary_data_offline/");?>",
	data: post_data
	}).done(function(reply) {
		//alert("Yesss");
...
$.post('<?= base_url("formdata/update_summary/");?>', $("#mform").serialize(), function(data) {
...
function calculateRow(){
	...
	$("#wo_rate").val(finalRate.toFixed( 2 ));
}
</script>
