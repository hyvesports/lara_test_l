...
<div class="row profile-page">
    ...
    <?php foreach($array1 as $key1 => $value1){ ?>
      ...
      $lastUpdateRow=$this->myaccount_model->get_last_updated_row_by_schedule_id_by_field($schedule_data['schedule_id'],$value1['summary_id'],'printing','fusing');
      //$any_rejection=$this->myaccount_model->check_any_rejection($schedule_data['schedule_department_id'],$value1['summary_id']);
      $any_rejection=$this->myaccount_model->check_any_rejection_by_qty_field($schedule_data['batch_number'],$schedule_data['schedule_id'],$value1['summary_id'],6);
      ...
      $mup='<span class="badge badge-success  w-100" >In Fusing</span>';
      $myUpdation=$this->myaccount_model->get_last_updated_row_by_schedule_id_by_field($schedule_data['schedule_id'],$value1['summary_id'],'fusing','bundling');
      if($myUpdation){
      $approvedQty=$value1['item_unit_qty_input']-$Rqty;
      if($myUpdation['verify_status']==1){
      if($actionFrom=="completed"){$rowNow="completed";}
      
      $mup='<span class="badge badge-warning mt-1 w-100" >In Bundling QC</span>';
      }
      ...
      //$not_submitted_count++;
      //$up='<a href="#" title="Update Status" style="cursor: pointer;"  data-toggle="modal" data-target="#orderItemUpdate"  data-sid="'.$schedule_data['schedule_id'].'"  data-sdid="'.$schedule_data['schedule_department_id'].'" data-smid="'.$value1['summary_id'].'" data-did="'.$staffRow['department_id'].'" data-act="up" ><label class="badge badge-primary w-100" style="cursor: pointer;">Update Status</label></a><br/>';
      }else{
      //$not_submitted_count++;
      $up='<a href="#" title="Update Status" style="cursor: pointer;"  data-toggle="modal" data-target="#orderItemUpdate"  data-sid="'.$schedule_data['schedule_id'].'"  data-sdid="'.$schedule_data['schedule_department_id'].'" data-smid="'.$value1['summary_id'].'" data-did="'.$staffRow['department_id'].'" data-act="up" ><label class="badge badge-primary w-100" style="cursor: pointer;">Update Status</label></a><br/>';
      }
      ...
      $st='<span class="badge badge-danger w-100" >In Printing</span>';
      }
      if($lastUpdateRow['verify_status']==0){
      //$not_submitted_count++;
      if($actionFrom=="pending"){$rowNow="pending";}
      $st='<span class="badge badge-info w-100" >In Printing</span>';
      }
      }else{
      if($actionFrom=="pending"){$rowNow="pending";}
      $st='<span class="badge badge-info w-100" >In Printing</span>';
      ...
      //}
      $info='<a href="#" title="All Updates" style="cursor: pointer;"  data-toggle="modal" data-target="#allUpdates"  data-sid="'.$schedule_data['schedule_id'].'"  data-sdid="'.$schedule_data['schedule_department_id'].'" data-smid="'.$value1['summary_id'].'" data-did="'.$staffRow['department_id'].'"  ><label class="badge badge-info w-100" style="cursor: pointer;">All Updates</label></a><br/>';
      ?>
      ...
      <div class="ml-3 mb-2"><strong><?php echo ucwords($value1['product_type']);?></strong><br/>
