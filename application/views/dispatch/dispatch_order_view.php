<div class="content-wrapper">
 <div class="row profile-page">
  <div class="col-12">
   <div class="card">
    <div class="card-body">
     <h4 class="card-title"><?php echo $title_head;?> <span style="text-align:right; float:right;"><a class="btn btn-primary " href="<?php echo base_url('myaccount/myorders');?>">List</a></span> </h4>
       ...
       <div class="col-md-12">
        ...
        ?>
            <tr>
             ...
             <td><strong>Sales Handler :</strong>
              <?php if($row['lead_id']==0){ echo 'Admin'; }else{ echo $row['sales_handler']; } ?></td>
            </tr>
          ...
          <ul>
           ...
           <li>
            ...
            <div class="details">
             ...
             <div class="buttons"> <a href="<?php echo $path_attachment;?>" class="view" target="_blank">View</a> <a href="<?php echo $path_attachment;?>" class="download" download>Download</a> </div>
            </div>
           </li>
           <?php } } ?>
          </ul>
  ...
  <div class="col-md-12 grid-margin stretch-card mt-4">
   <div class="card">
	...
	if($actionFrom=="competed"){ $actionFrom="competed";}else if($actionFrom=="pending"){ $actionFrom="pending"; }else{ $actionFrom="all"; }?>
        <?php foreach($array1 as $key1 => $value1){ $approvedQty=0; ?>
        <!--************************************************************************************************************************-->
        <?php
		$up="";
		$rowNow="all";
		if($row['is_re_scheduled']>0){$re='<span class="badge badge-outline-warning w-10" ><strong>Re Scheduled</strong></span>';}
		 $lastUpdateRow=$this->myaccount_model->get_last_updated_row_by_field($schedule_data['schedule_department_id'],$value1['summary_id'],'stitching','final_qc');
		//$any_rejection=$this->myaccount_model->check_any_rejection($schedule_data['schedule_department_id'],$value1['summary_id']);
		//$any_rejection=$this->myaccount_model->check_any_rejection_by_qty($schedule_data['batch_number'],$schedule_data['schedule_id'],$value1['summary_id'],5);
		$isCompleted=$this->common_model->check_is_a_completed_by_field($lastUpdateRow['rs_design_id'],$value1['summary_id']);
			if(isset($lastUpdateRow)){
				if($lastUpdateRow['verify_status']==1){
					
							if($lastUpdateRow['verify_status']==1){
								 $st='<span class="badge badge-outline-success w-100" >Final QC Approved</span>';
								if($lastUpdateRow['accounts_status']==1){
									 $acc='<br><span class="badge badge-outline-success  text-info w-100  mt-1" >Account Dept: Approved </span>';
									
								}else{
									 $acc='<br><span class="badge badge-outline-danger text-danger  w-100  mt-1" >Account Dept:<br>Not Approved</span>';
								}
							}
							if($lastUpdateRow['verify_status']==-1){
							$st='<span class="badge badge-outline-danger mt-1" >Rejected</span>';
							}
							if($lastUpdateRow['verify_status']==0){
							$st='<span class="badge badge-outline-warning mt-1" >Not Approved</span>';
							}
				...
				if($lastUpdateRow['verify_status']==0){
					...
					$st='<span class="badge badge-danger w-100" >QC Not Approved</span>';
				}
			}else{
				if($actionFrom=="pending"){$rowNow="pending";}
				$st='<span class="badge badge-danger w-100" >QC Not Approved</span>';
        ...
        <tr>
         <td class="py-1 pl-0" valign="top"><div class="d-flex align-items-center">
...
<span class="btn btn-xs btn-rounded btn-outline-primary mr-2 mb-1"><?php echo ucwords($value1['sleeve_type']);?></span><br/>
 <span class="btn btn-xs btn-rounded btn-outline-primary mr-2"><?php echo ucwords($value1['fabric_type']);?></span>
            ...
            <div id="lightgallery2" class="row lightGallery"><a href="<?php echo $value1['img_front'];?>" target="_blank"><img src="<?php echo $value1['img_front'];?>" title="Image Front" width="100" height="100"  style="width:100px; height:100px;"><br/>
             Front</a></div>
            </span>
            <?php } ?>
            <?php if($value1['img_back']!=""){?>
            <span class="btn btn-xs  btn-outline-primary mr-2 mt-1">
            <div id="lightgallery2" class="row lightGallery"><a href="<?php echo $value1['img_back'];?>" target="_blank"><img src="<?php echo $value1['img_back'];?>" title="Image Back" width="100" height="100" style="width:100px; height:100px;" ><br/>
             Back</a></div>
            </span>
            <?php } ?>
            </span> </div>
         ...
         <td>
					...
					<?php if($isCompleted['qc_approved_qty']!=""){ ?>
					<label class="badge badge-info"><?php echo ucwords($isCompleted['qc_approved_qty']);?></label>
					<?php }else{ ?>
					<label class="badge badge-danger"><?php echo ucwords($value1['item_unit_qty_input']);?></label>
					<?php } ?>


          <?php if($approvedQty>0){ echo '<label class="badge badge-success" title="Approved Quantity" >'.$approvedQty.'</label>'; } ?>
          <?php if($Rqty>0){ echo '<label class="badge badge-danger" title="Rejected Quantity" >'.$Rqty.'</label>'; } ?></td>
         <td style="font-size:10px;"><?php if($value1['remark']!=""){?>
          <div class="alert alert-info" role="alert" style="font-size:10px;"><?php echo ucwords($value1['remark']);?></div>
          <?php } ?>
          <?php 
          ...
          <div class="alert alert-success" role="alert"> Design URL : <a href="<?php echo $design_result['response_url'];?>" target="_blank"><?php echo $design_result['response_url'];?></a> </div>
          <?php
            }
            ?>
         
					</td>
         <td class="text-center"><?php echo $rejst;?> <?php echo $re;?> <?php echo $st.$acc;?></td>
         <td><?php echo $up;?> <a href="#" class="badge badge-warning mt-1 mb-1 float-center w-100" title="Item Info" style="cursor: pointer;"  data-toggle="modal" data-target="#itemInfo"  data-sid="<?php echo $schedule_data['schedule_id'];?>"  data-sdid='<?php echo $schedule_data['schedule_department_id'];?>' data-smid='<?php echo $value1['summary_id'];?>' data-did='<?php echo $staffRow['department_id'];?>' data-act='list' >Item Info</a><br />
          <?php //echo $info;?></td>
        </tr>
        <?php } ?>
        <?php //} ?>
