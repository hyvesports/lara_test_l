...
//        echo $sql;
    ...
    <thead>
    <tr>
    <th width="50%" class="pt-1 pl-0">Product</th> <th width="10%" class="pt-1">Qty</th>
    ...
    foreach($array1 as $key1 => $value1){ $approvedQty=0;
    $uploadImg="";
    $st="";
    $lastUpdateRow=$this->myaccount_model->get_last_updated_row_by_schedule_id_by_field($schedule_data['schedule_id'],$value1['summary_id'],'stitching','final_qc');
    //print_r($lastUpdateRow);
    $any_rejection=$this->myaccount_model->check_any_rejection_by_qty_field($schedule_data['batch_number'],$schedule_data['schedule_id'],$value1['summary_id'],13);
    ...
    $st='<br><span class="badge badge-outline-success w-100" >In Accounts</span>';
    $st.='<br><span class="badge badge-outline-success mt-1 w-100 " >Account Dept:<br>Approved </span>';
    }else{
    $st='<br><span class="badge badge-outline-success w-100" >In Accounts</span>';
    $st.='<br><span class="badge badge-outline-danger mt-1 w-100" >Account Dept:<br>Not Approved </span>';
    }
    }
    $approvedQty=0;
    if($lastUpdateRow['verify_status']==-1){
    ...
    $st='<span class="badge badge-warning w-100 mt-1" >In Stitching</span>';
    }
    $uploadImg='<a href="#" title="All Updates" style="cursor: pointer;"  data-toggle="modal" data-target="#fqcImage"  data-sid="'.$row['schedule_id'].'"  data-sdid="'.$row['schedule_department_id'].'" data-smid="'.$value1['summary_id'].'" data-did="'.$staffRow['department_id'].'"  ><label class="badge badge-success w-100" style="cursor: pointer;"><i class="fa fa-image" ></i> Upload Images</label></a><br/>';
    
    $info='<a href="#" title="All Updates" style="cursor: pointer;"  data-toggle="modal" data-target="#allUpdates"  data-sid="'.$schedule_data['schedule_id'].'"  data-sdid="'.$schedule_data['schedule_department_id'].'" data-smid="'.$value1['summary_id'].'" data-did="'.$staffRow['department_id'].'"  ><label class="badge badge-info w-100" style="cursor: pointer;">All Updates</a>';
    ?>
...
$(function() {
	...
	$('#fqcImage').on('show.bs.modal', function (event) {
	...
	//alert(act);
	var formData = "<?php echo $this->security->get_csrf_token_name(); ?>=<?php echo $this->security->get_csrf_hash(); ?>&sid="+sid+"&sdid="+sdid+"&smid="+smid+"&did="+did;
	...
	$.ajax({  
	type: "POST",
	url: "<?= base_url("finalqc/upload_image/");?>",  
	data: formData,
	beforeSend: function(){
	...
	$.ajax({  
	type: "POST",
	url: "<?= base_url("finalqc/final_qc_approve_denay/");?>",  
	data: formData,
	beforeSend: function(){
		modal.find('.modal-content').html('<i class="fa fa-spin fa-refresh"></i>');},
		success: function(response){
...
<div class="modal" id="fqcImage" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel" aria-hidden="true">
<div class="modal-dialog modal-md" role="document" style="max-width: 700px;">
<div class="modal-content"></div> 
</div>
</div>
