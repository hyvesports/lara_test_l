...
<div class="row profile-page">
   ...
   <?php $options=$this->myaccount_model->get_order_options($row['order_id'],$value1['summary_id']); ?>
    ...
    <?php
	//echo 1;
	$qc_result=$this->stitching_model->check_bundling_deptmt_approved($value1['summary_id'],'bundling');
	if($qc_result['rs_design_id']==""){ ?>
    <div class="alert alert-danger" role="alert">Bundling Department Not Approved</div>
    
    <?php }else{ ?>
    <div class="alert alert-success" role="alert">Bundling Department Approved on <?php echo $qc_result['verify_datetime'];?>.</div>
    <?php $design_result=$this->myaccount_model->get_design_data($value1['summary_id'],'design'); ?>
	...
	?>
        ...
        and qc_name='stitching' ORDER BY rs_design_id DESC LIMIT 1 ";
        //echo $sel;
        $query = $this->db->query($sel);					 
        $rsRow2=$query->row_array();
        ?>
        <?php if($rsRow2!=""){ ?>
        <?php if($rsRow2['verify_status']==0){ echo '<div class="badge badge-warning">Order Submitted</div>'; }?>
        <?php if($rsRow2['verify_status']==1){ echo '<div class="badge badge-success">Order Approved</div>'; }?>
        <?php if($rsRow2['verify_status']==-1){ echo '<div class="badge badge-danger">Order Rejected</div>'; }?>
        <?php } ?>
