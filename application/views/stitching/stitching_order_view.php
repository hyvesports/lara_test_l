...
<div class="row profile-page">
    ...
    <p class="card-description" style="border-bottom: 1px solid #f2f2f2;"><strong>Scheduled Details</strong></p>
        ...
        <tr>
        <td>
        <strong>My Scheduled Date :</strong> <?php echo date("d-m-Y", strtotime($rsRow['department_schedule_date'])); ?></td>
    ...
    <div class="card">
    <div class="card-body">
    <h4 class="card-title">Order Items dd</h4>
    ...
    $re="";
    $lastUpdateRow=$this->myaccount_model->get_last_updated_row_by_schedule_id_by_field($schedule_data['schedule_id'],$value1['summary_id'],'fusing','bundling');
    $any_rejection=$this->myaccount_model->check_any_rejection_by_qty_field($schedule_data['batch_number'],$schedule_data['schedule_id'],$value1['summary_id'],8);
    ...
    $subm=0;
    $st='<span class="badge badge-success w-100" >In Stitching</span>';
    
    $myUpdation=$this->myaccount_model->get_last_updated_row_by_schedule_id_by_field($schedule_data['schedule_id'],$value1['summary_id'],'stitching','final_qc');
    if($myUpdation){
    if($myUpdation['verify_status']==1){
    //$approvedQty=$value1['item_unit_qty_input']-$Rqty;
    $st='<br/><span class="badge badge-warning mt-1 w-100" >In Final QC</span>';
    $approved_count++;
    }
    ...
    $st='<span class="badge badge-warning w-100" >In Final QC</span>';
    //$not_submitted_count++;
    }
    }else{
    
    $st='<span class="badge badge-warning w-100" >In Stitching</span>';
    //$not_submitted_count++;
    }
    }
    //$schedule_data['is_re_scheduled']."asdafasf";
    if($schedule_data['is_re_scheduled']!=0){
    $up='<a href="#" title="Update Status" style="cursor: pointer;"  data-toggle="modal" data-target="#orderItemUpdate"  data-sid="'.$schedule_data['schedule_id'].'"  data-sdid="'.$schedule_data['schedule_department_id'].'" data-smid="'.$value1['summary_id'].'" data-did="'.$staffRow['department_id'].'" data-act="up" ><label class="badge badge-primary w-100" style="cursor: pointer;">Update Status</label></a><br/>';
    }
    ...
    <?php //echo $schedule_data['schedule_department_id'];?>
