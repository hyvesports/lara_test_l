...
class Stitching_model extends CI_Model{
	//
	public function get_works_stitching($did,$unit_managed,$type){
			$cdate=date('Y-m-d');
			$wh =array();
			$SQL='SELECT 
				SH.schedule_uuid,
...
SHD.total_order_items as TOTAL_COUNT
			...
			if($type=="pending"){
				$wh[] = 'FIND_IN_SET('.$did.',SHD.department_ids) AND SHD.unit_id IN('.$unit_managed.') AND SHD.department_schedule_date <"'.$cdate.'"
			and SHD.total_order_items!=(SELECT count(*) FROM rs_design_departments as T11 WHERE T11.schedule_department_id=SHD.schedule_department_id and T11.is_current=1 and T11.from_department="stitching") ';
			}
			if($type=="completed"){
				$wh[] = 'FIND_IN_SET('.$did.',SHD.department_ids) AND SHD.unit_id IN('.$unit_managed.')
			and SHD.total_order_items=(SELECT count(*) FROM rs_design_departments as T11 WHERE T11.schedule_department_id=SHD.schedule_department_id and T11.is_current=1 and T11.from_department="stitching") ';
	...
	public function get_completed_works_______($did,$unit_managed){
			$wh =array();
			$SQL='SELECT 
				SH.*,
				SHD.department_schedule_date,
	...
	public function get_pending_works____($did,$unit_managed){
			...
			RDD.verify_status=1  AND  RDD.status_value=1 AND RDD.submitted_to_stitching=0';
...
//-----------------------------------------------------------------------------	
	...
	public function get_submitted_works($did,$unit_managed){
			...
			$wh[] = ' RDD.submitted_by="'.$this->session->userdata('loginid').'" and RDD.status_value=1 AND RDD.qc_name="stitching"';
