...
class Fusing extends CI_Controller {
	...
	public function list_completed(){
		...
		$records = $this->fusing_model->get_completed_works($staffRow['department_id'],$staffRow['unit_managed']);
		$data = array();
		...
		foreach ($records['data']  as $row) 
		{
			...
			if($array1){
				...
				foreach($array1 as $key1 => $value1){
					...
					$lastUpdateRow=$this->myaccount_model->get_last_updated_row_by_schedule_id($row['schedule_id'],$value1['summary_id'],'printing','fusing');
					...
					if(isset($any_rejection)){
						...
						if(isset($lastUpdateRow)){
							...
							//$st='<span class="badge badge-outline-success" >Printing Dept:<br/> Submitted</span>';
							
							$sub=1;
								$myUpdation=$this->myaccount_model->get_last_updated_row_by_schedule_id($row['schedule_id'],$value1['summary_id'],'fusing','bundling');
								if($myUpdation){
									if($myUpdation['verify_status']==1){
									$mup='<br/><span class="badge badge-outline-warning mt-1" >Bundling QC :<br/>'.ucwords($myUpdation['row_status']).'</span>';
									}
									if($myUpdation['verify_status']==0){
									$mup='<span class="badge badge-outline-warning mt-1" >'.ucwords($myUpdation['row_status']).'<br/> To Bundling QC :</span>';
									}
								}else{
							...
							if($lastUpdateRow['verify_status']==-1){
							//$st='<span class="badge badge-outline-danger" >Printing Dept: <br/>Not Submitted</span>';
							}
							if($lastUpdateRow['verify_status']==0){
							//$st='<span class="badge badge-outline-danger" >Printing Dept: <br/>Not Submitted</span>';
							}
						}else{
							//$st='<span class="badge badge-outline-warning" >Printing Dept: <br/>Not Submitted</span>';
						}
					}
					...
					$data[]= array(
						...
						$sales_handler,
						'<label class="badge badge-outline-success">'.$row['wo_product_info'].$ref.$value1['product_type'].'-'.$value1['item_unit_qty_input'].'</label>',
						$st.$mup,
						$info.$up.$option
					);
					}
	...
	public function list_pending(){
		...
		$staffRow=$this->myaccount_model->get_staff_profile_data($loginid);
		//echo $staffRow['department_id'];
		$records = $this->fusing_model->get_pending_works_latest($staffRow['department_id'],$staffRow['unit_managed']);
		$data = array();
						...
						if(isset($lastUpdateRow)){
							...
							$sub=1;
								...
								if($myUpdation){
									...
									if($myUpdation['verify_status']==0){
									$mup='<br/><span class="badge badge-outline-warning mt-1" >'.ucwords($myUpdation['row_status']).'<br/> To Bundling QC :</span>';
									}
								}else{
$sub=1;
									$up='&nbsp;<a href="#" title="Update Status" style="cursor: pointer;"  data-toggle="modal" data-target="#orderItemUpdate"  data-sid="'.$row['schedule_id'].'"  data-sdid="'.$row['schedule_department_id'].'" data-smid="'.$value1['summary_id'].'" data-did="'.$staffRow['department_id'].'" data-act="up" ><label class="badge badge-primary" style="cursor: pointer;"><i class="fa fa-refresh" ></i></label></a>';
...
$sub=1;
	...
	public function list_all(){
		...
		$staffRow=$this->myaccount_model->get_staff_profile_data($loginid);
		//echo $staffRow['department_id'];
		$records = $this->fusing_model->get_all_works($staffRow['department_id'],$staffRow['unit_managed']);
		$data = array();
	...
	public function list_active(){
		...
		$staffRow=$this->myaccount_model->get_staff_profile_data($loginid);
		//echo $staffRow['department_id'];
		$records = $this->fusing_model->get_active_works($staffRow['department_id'],$staffRow['unit_managed']);
		$data = array();
...
//---------------------------------------------------------------------------------------------------------------------------------------------------------------//	
	...
	public function order_view($uuid,$sdid,$rs_design_id){
		...
		if($staffRow['department_id']==6){
			$data['request_row']=$this->fusing_model->get_fusing_request_row($rs_design_id);
			$data['view']='fusing/fusing_order_view_single';
	...
	public function order_submitted(){
		...
		$loginid=$this->session->userdata('loginid');
		$staffRow=$this->myaccount_model->get_staff_profile_data($loginid);
		$records = $this->fusing_model->get_submitted_works($staffRow['department_id'],$staffRow['unit_managed']);
		$data = array();
		...
		foreach ($records['data']  as $row) 
		{
			...
			$option3.='<a href="'.base_url('fusing/order_view/'.$row['schedule_uuid']).'/'.$row['schedule_department_id'].'/'.$row['rs_design_id'].'" title="View" style="cursor: pointer;"><label class="badge badge-info" style="cursor: pointer;"><i class="fa fa-search" ></i> View</label></a>';
	...
	public function list_pending_old(){
		...
		//echo $staffRow['department_id'];
		$records = $this->fusing_model->get_pending_works($staffRow['department_id'],$staffRow['unit_managed']);
		$data = array();
	...
	public function save_updates(){
		if($this->input->post('submitData')){
			...
			if ($this->form_validation->run() == FALSE) {
				...
				$selPre="SELECT * FROM rs_design_departments WHERE schedule_id='$schedule_id' AND summary_item_id='$summary_id' and unit_id='$unit_id' and verify_status=1 and row_status='submitted' and to_department='fusing' and fusing_submitted=0 ORDER BY rs_design_id DESC LIMIT 1 ";
				$queryPre = $this->db->query($selPre);			 
    			$rsRowPre=$queryPre->row_array();
				if($rsRowPre){
					$rs_design_id=$rsRowPre['rs_design_id'];
					$upPre="UPDATE rs_design_departments SET fusing_submitted='1' WHERE rs_design_id='$rs_design_id' ";
					$this->db->query($upPre);
				}
				...
				'qc_name'=>'fusing',
				'from_department'=>'fusing',
				'to_department'=>'bundling',
				'row_status'=>'submitted',
				'log_info'=>$log_info
				);
				//print_r($insertNew);exit;
				$sel="SELECT * FROM rs_design_departments WHERE schedule_department_id='$schedule_department_id'  AND summary_item_id='$summary_id' and qc_name='fusing' ORDER BY rs_design_id DESC LIMIT 1 ";
				//echo $sel;
				...
				if($rsRow['status_value']==1 ){
					if($rsRow['verify_status']==1){
						...
						$log_data = array(
							'log_title' => 'Work order submitted ['.$log_info.'] to bundling.',				  
							'log_timestamp'=>$now,
							'log_controller' => 'fusing',
							'log_function' => 'save_updates',
							'log_module' => 'Fusing Submit',
							'log_desc' => $log_desc,
							'access_module_code'=>'fusing_submitted',
