...
class Fusing extends CI_Controller {
	...
	public function list_works_fusing($type){
		...
		$records = $this->fusing_model->get_works_fusing($staffRow['department_id'],$staffRow['unit_managed'],$type);
		$data = array();
		foreach ($records['data']  as $row) 
		{
			...
			$st='<a href="#" class=" mt-1 mb-1 float-center w-100"  style="cursor: pointer;"  data-toggle="modal" data-target="#statusInfo"  data-sid="'.$row['schedule_department_id'].'" data-fd="fusing" data-did="'.$staffRow['department_id'].'">View Status</a>';
		...
		foreach ($records['data']  as $row) 
		{
			...
			if($array1){
				...
				foreach($array1 as $key1 => $value1){
					...
					$any_rejection=$this->myaccount_model->check_any_rejection_by_qty($row['batch_number'],$row['schedule_id'],$value1['summary_id'],6);
