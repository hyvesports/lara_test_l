...
class Stitching extends CI_Controller {
	...
	public function list_works_stitching($type){
		...
		$records = $this->stitching_model->get_works_stitching($staffRow['department_id'],$staffRow['unit_managed'],$type);
		$data = array();
		foreach ($records['data']  as $row) 
		{
			...
			$st='<a href="#" class=" mt-1 mb-1 float-center w-100"  style="cursor: pointer;"  data-toggle="modal" data-target="#statusInfo"  data-sid="'.$row['schedule_department_id'].'" data-fd="stitching" data-did="'.$staffRow['department_id'].'">View Status</a>';
			if($row['wo_product_info']!=""){
				...
				$quicInfo='<a href="#" class=" mt-1 mb-1 float-center w-100"  style="cursor: pointer;"  data-toggle="modal" data-target="#scheduleInfo"  data-sid="'.$row['schedule_department_id'].'" title="'.$row['wo_product_info'].'" >Nil</a>';
			}
				...
				if($rsRow['status_value']==1 ){
					...
					if($rsRow['verify_status']==1){
						...
						if($unitTypeArray['orderform_type_id'] ==2)
							...
							$scheduleSmsArray=$this->workorder_model->get_online_order_sms_status($order_id,$summary_id,'stiching_sms');
...
error_log("coming here===",0);
									...
									if($customer_phone_no!='')
									    {
										$sms_status=$this->msg91->send_online_stiching_order_sms($customer_phone_no, $wo_ref_no);
...
//											error_log("Sms Creation Status===".$smsCreationStatus);
						...
						if($unitTypeArray['orderform_type_id'] ==1)
							...
							$scheduleSmsArray=$this->workorder_model->get_order_sms_status($order_id,'stiching_sms');
							$chkSmsDataAvailable=$this->workorder_model->get_offline_order_sms_completed($order_id);
							if($customerArray)
							    {
								if($scheduleSmsArray['cnt'] < 1)
								    {
//error_log("count status===".$scheduleSmsArray['cnt'],0);
									$customer_phone_no=$customerArray['customer_mobile_no'];
									$orderform_number=$customerArray['orderform_number'];
									if($customer_phone_no!='')
									    {
										$sms_status=$this->msg91->send_offline_stiching_order_sms($customer_phone_no,$orderform_number);
//										error_log("Sms Status=====". $sms_status,0);
										if($sms_status==1)
											...
											if($chkSmsDataAvailable['cnt'] > 0)
												...
												$smsCreationStatus=$this->workorder_model->update_sms_wo_data($order_id,0 ,$sdataSms);
											    }
