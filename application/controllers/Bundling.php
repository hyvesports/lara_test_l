...
class Bundling extends CI_Controller {
	...
	public function list_works_bundling($type){
		...
		$records = $this->bundling_model->get_works_bundling($staffRow['department_id'],$staffRow['unit_managed'],$type);
		$data = array();
		foreach ($records['data']  as $row) 
		{
			...
			$st='<a href="#" class=" mt-1 mb-1 float-center w-100"  style="cursor: pointer;"  data-toggle="modal" data-target="#statusInfo"  data-sid="'.$row['schedule_department_id'].'" data-fd="bundling" data-did="'.$staffRow['department_id'].'">View Status</a>';
		...
		if($this->input->post('submit')){
			...
			if($verify_status==-1){
				...
				//if($approvedQty>0){
					...
					'item_rejected_qty' =>$item_rejected_qty,
					'item_re_schedule_id' =>$item_re_schedule_id,
					'item_unit_qty_input' =>$item_unit_qty_input,
					));
					$submitted_item_updated=json_encode($new_item_array);
					//print_r($submitted_item_updated);
					if($approvedQty!=0){
					$sqlApproved="UPDATE rs_design_departments SET approved_dept_id='12',verify_status='1',verify_remark='$verify_remark',verify_datetime='$verify_datetime',approved_dep_name='$qc_name',approved_by='$approved_by',row_status='approved',submitted_item='$submitted_item_updated' WHERE rs_design_id='$rs_design_id' ";
					}else{
					$sqlApproved="UPDATE rs_design_departments SET approved_dept_id='12',verify_status='-1',verify_remark='$verify_remark',verify_datetime='$verify_datetime',approved_dep_name='$qc_name',approved_by='$approved_by',row_status='approved',submitted_item='$submitted_item_updated' WHERE rs_design_id='$rs_design_id' ";
					}
					//echo $sqlApproved;exit;
					...
					$orRowData=$query3->row_array();
						...
						foreach($deptments as $DP){
							...
							$insert= array(
								...
								'is_re_scheduled'=>$rsRowData['schedule_id']
							);
							$this->schedule_model->save_reschedule_data($insert);
							//print_r($insert);
						} 
						$ins_rej=array(
						'schedule_id'=>$shRowData['schedule_id'],
						'order_id'=>$shRowData['order_id'],
						'batch_number'=>$batch_number,
						'schedule_department_id'=>$rsRowData['schedule_department_id'],
						'approved_to_departments'=>'7,8,9,10,13',
						'rejected_to_departments'=>'4,5,6,11,12',
						'summary_id'=>$summary_id,
						'rejected_qty'=>$rej_qty,
						'approved_qty'=>$approvedQty,
						'rejected_remark'=>$verify_remark,
						'rejected_from_departments'=>'Bundling QC',
						'rejected_by'=>$this->session->userdata('loginid'),
						'rejected_datetime'=>$verify_datetime
						);
						...
						if($recipient['ph_users']!=""){ // staff id
							...
							$notification_title="Rejection in orders [Bundling QC]";
				...
				$sql="UPDATE rs_design_departments SET approved_dept_id='12',verify_status='$verify_status',verify_remark='$verify_remark',verify_datetime='$verify_datetime',
					approved_dep_name='$qc_name',approved_by='$approved_by',row_status='approved' WHERE rs_design_id='$rs_design_id' ";
				$query = $this->db->query($sql);
				$parent_rs_design_id=$this->input->post('parent_rs_design_id'); // incase of fully reject order re approval
				if($parent_rs_design_id!=0){
					$upp="UPDATE rs_design_departments SET approved_dept_id='12',verify_status='$verify_status',verify_remark='$verify_remark',verify_datetime='$verify_datetime',
					approved_dep_name='$qc_name',approved_by='$approved_by',row_status='approved' WHERE rs_design_id='$parent_rs_design_id' ";
					$query = $this->db->query($upp);
				}
			}
		...
		foreach ($records['data']  as $row) 
		{
			...
			if($array1){
				...
				foreach($array1 as $key1 => $value1){
					...
					$re="";
					if($row['is_re_scheduled']>0){ $re='&nbsp;<span class="badge badge-outline-warning" ><strong>R</strong></span>';}if($row['is_re_scheduled']>0){
						$re='&nbsp;<span class="badge badge-outline-warning" ><strong>R</strong></span>';
					...
					$any_rejection=$this->myaccount_model->check_any_rejection_by_qty($row['batch_number'],$row['schedule_id'],$value1['summary_id'],12);
			...
			$recordsListCount++;
				$data[]= array(
				'<td><span class="badge" style="background-color:'.$row['priority_color_code'].';" >'.date("d-m-Y", strtotime($row['department_schedule_date'])).'</span></td>',
				'<td><span class="badge badge-success" >'.$row['orderform_number'].' ['.$i.']'.$scode.'</span>'.$re.'</td>',
				'<td><span class="badge" >'.substr($row['wo_date_time'],0,10).'<br/>'.substr($row['wo_date_time'],10,10).'</span></td>',
				date("d-m-Y", strtotime($dispatchRow['department_schedule_date'])),
				$sales_handler,
