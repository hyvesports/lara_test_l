			...
			if($verify_status==-1){
				...
				$submitted_item=$rsRowData['submitted_item'];
					...
					if($approvedQty!=0){
						...
						$sqlApprove="UPDATE rs_design_departments SET approved_dept_id='13',verify_status='$verify_status',verify_remark='$verify_remark',
...
submitted_to_accounts='$submitted_to_accounts',accounts_status='$accounts_status',accounts_verified_by='$accounts_verified_by',row_status='approved'
 WHERE rs_design_id='$rs_design_id' ";
						...
						$sqlRejection="UPDATE rs_design_departments SET approved_dept_id='13',verify_status='$verify_status',verify_remark='$verify_remark',verify_datetime='$verify_datetime',
rejected_department='final qc',rejected_by='$approved_by',row_status='rejected' WHERE rs_design_id='$rs_design_id' ";
					...
					$sqlSel4="SELECT my_child_departments FROM `department_master` WHERE department_id=13 "; 
					...
					if($reject_dep_id==4){
						...
						foreach($deptments as $DP){
							$insert= array('schedule_id'=>$new_schedule_id,'department_ids'=>$DP,'department_schedule_date' =>date('Y-m-d'),'department_schedule_status' => 0,'scheduled_order_info' => json_encode($new_item_array),'unit_id'=>$shRowData['schedule_unit_id'],'order_id'=>$shRowData['order_id'],'batch_number'=>$new_batch_number,'is_re_scheduled'=>$rsRowData['schedule_id']);
							$this->schedule_model->save_reschedule_data($insert);
						}
						$ins_rej=array('schedule_id'=>$shRowData['schedule_id'],'order_id'=>$shRowData['order_id'],'batch_number'=>$batch_number,'schedule_department_id'=>$rsRowData['schedule_department_id'],'approved_to_departments'=>'','rejected_to_departments'=>'4,5,6,11,12,7,8,9,10,13','summary_id'=>$summary_id,'rejected_qty'=>$rej_qty,
...
`
				...
				if($orderform_type_id==1){
					...
					SET 
						approved_dept_id='13',verify_status='$verify_status',
