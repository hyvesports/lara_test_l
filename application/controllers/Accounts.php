...
class Accounts extends CI_Controller {
	...
	public function save_updates(){
		if($this->input->post('submit')){
			...
			$wo_accounts_verified_datetime = date('Y-m-d H:i:s');
			...
			$odata=array('accounts_completed_status'=>1,'accounts_completed_date'=>$wo_accounts_verified_datetime);
			$this->common_model->update_wo_completion_status($odata,$oid);
	...
	public function orders_list($currentList= null){
		  $accessArray=$this->rbac->check_operation_access(); 
		$order_type=1;
		if($currentList=="all"){
			$selectBy="all";
		}else if($currentList=="approved"){
			$selectBy='approved';
		}else if($currentList=="approve"){
			$selectBy='approve';
		}else{
			$selectBy="all";
		}

		  $records = $this->accounts_model->get_all_completed_works($order_type,$selectBy);
		  $data = array();
		  $i=0;
		  foreach ($records['data']  as $row) 
		  {  
			if($row['production_completed_status']==1){
				if($row['accounts_completed_status']==1){
				$status='<label class="badge badge-success w-100" style="cursor: pointer;"><i class="fa fa-thumbs-up" ></i> Approved</label>';
				}else{
$status='<a href="#" title="Approve" style="cursor: pointer;" data-toggle="modal" data-target="#approveDeny" data-rdid="'.$row['last_rs_design_id'].'" data-oid="'.$row['order_id'].'" data-afor="1"><label class="badge badge-warning w-100" style="cursor: pointer;"><i class="fa fa-thumbs-up" ></i> Approve</label></a>';
				}
			}			

			$option='
<a href="'.base_url('accounts/view/'.$row['order_uuid']).'" title="View" style="cursor: pointer;"><label class="badge badge-success" style="cursor: pointer;"><i class="fa fa-search" ></i></label></a>';
...
//___________________________________
	...
	public function view_old($uuid,$rdid){
		$accessArray=$this->rbac->check_operation_access(); // check opration permission
			if($accessArray==""){
				redirect('access/not_found');
			}
	...
	public function orders_list_old(){
		$accessArray=$this->rbac->check_operation_access(); 
